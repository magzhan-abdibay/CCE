...
void AGoal::ResetBallPosition(AActor* OtherActor) const
{
	OtherActor->Destroy();
                         ...
                         bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor->IsA(ABall::StaticClass()))
	{
		...
		ABall* Ball = static_cast<ABall*>(OtherActor);
		AAgent* LastKickedAgent = Ball->GetLastKickedAgent();
		if(Ball!=nullptr && LastKickedAgent!=nullptr)
		{
			if (Team!=LastKickedAgent->GetTeam() )
			{
				LastKickedAgent->IncrementScoredPoints();
			}
			else
			{
				LastKickedAgent->DecrementScoredPoints();
			}
		}
