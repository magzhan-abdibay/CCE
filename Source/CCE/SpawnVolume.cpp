...
* */
NEAT::Population* ASpawnVolume::pole2TestRealTime() {
	cout << "START DOUBLE POLE BALANCING REAL-TIME EVOLUTION VALIDATION" << endl;
	cout << "Reading in the start genome" << endl;
	char curWord[20];
	int id;
	ifstream iFile("E:\\UnrealProjects\\CCE\\NEAT\\NEATConsoleApplication\\pole2startgenes1", ios::in);
	iFile >> curWord;
	iFile >> id;
	...
	NEAT::Population *pop = new NEAT::Population(startGenome, NEAT::popSize);
...
int ASpawnVolume::pole2RealTimeLoop(NEAT::Population *pop, CartPole *cart) {
	int pause;
	bool win = false;
	...
	for (int offspringCount = 0; offspringCount < 20000; offspringCount++) {
		...
		NEAT::Organism *newOrg = (pop->chooseParentSpecies())->reproduceOne(offspringCount, pop, pop->species);
...
bool ASpawnVolume::pole2Evaluate(NEAT::Organism *org, CartPole *cart) {
	NEAT::Network *net;
