...
ASpectatorCamera::ASpectatorCamera()
{
	...
	CameraSpringArmComponent->SetRelativeLocationAndRotation(
		FVector(0.0f, 0.0f, 50.0f), FRotator(0.0f, 0.0f, 0.0f));
...
void ASpectatorCamera::BeginPlay() { Super::BeginPlay(); }
...
{
	// Rotate our actor's yaw, which will turn our camera because we're attached
	// to it
...
void ASpectatorCamera::HandlePitch()
{
	...
	NewRotation.Pitch =
		FMath::Clamp(NewRotation.Pitch + CameraInput.Y, -80.0f, 80.0f);
...
void ASpectatorCamera::PitchCamera(float Value) { CameraInput.Y = Value; }

void ASpectatorCamera::YawCamera(float Value) { CameraInput.X = Value; }
